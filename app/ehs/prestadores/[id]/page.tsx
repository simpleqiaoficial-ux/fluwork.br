...
import { listarCarteirinhasPrestadorEhs } from "@/app/actions/ehs-carteirinhas"
...
export default async function PrestadorEhsDetailPage({ params }: PrestadorEhsDetailPageProps) {
  ...
  const [documentosPorTipo, timeline, auditoria, integracoes, carteirinhas] = await Promise.all([
    ...
    listarCarteirinhasPrestadorEhs(id),
  ])
  ...
  return (
    <div className="container mx-auto py-8 px-4 lg:px-6 max-w-6xl">
      ...
      <PrestadorDetailTabs
        ...
        compliance={prestador.compliance}
      />
    </div>
  )
}
